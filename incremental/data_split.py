...
def load_raw_data(dataset_source):
    ...
    if dataset_source=='cora-full':
        dataset=torch_geometric.datasets.CoraFull('./cache')[0]
        n1s=dataset.edge_index[0]
        n2s=dataset.edge_index[1]
    elif dataset_source=='reddit':
        dataset=torch_geometric.datasets.Reddit('./cache')[0]
        n1s=dataset.edge_index[0]
        n2s=dataset.edge_index[1]
    else:
        n1s = []
    ...
    if dataset_source not in ['dblp','Amazon_clothing']:
        labels=np.array(dataset.y.unsqueeze(1))
        features=np.array(dataset.x)
    else:
...
def mutual_information(embeddings):
    joint_distribution = torch.mm(embeddings, embeddings.t()) / embeddings.size(1)

    marginal_distribution_x = torch.mean(embeddings, dim=1, keepdim=True)
    marginal_distribution_y = marginal_distribution_x.t()

    mutual_info = torch.sum(
        joint_distribution * torch.log(joint_distribution / (marginal_distribution_x * marginal_distribution_y)))

    return mutual_info
def calculate_distance_sum(embeddings):
    distances = torch.norm(embeddings[:, None] - embeddings, dim=2, p=2)
...
def embedding_distance_variance(embeddings):
    mean_embedding = torch.mean(embeddings, dim=0)

    distances = torch.norm(embeddings - mean_embedding, p=2, dim=1)

    variance = torch.var(distances)

    return variance
def calculate_l2_norm_sum(embeddings):
    inner_product_matrix = torch.mm(embeddings, embeddings.t())

    num_embeddings = embeddings.size(0)
    identity_matrix = torch.eye(num_embeddings, device=embeddings.device)
    centered_matrix = inner_product_matrix - identity_matrix

    l2_norm_sum = torch.norm(centered_matrix, p='fro')

    return l2_norm_sum
def calculate_distance_variance(embeddings):
    num_embeddings = embeddings.size(0)

    pairwise_distances = torch.norm(embeddings.unsqueeze(0) - embeddings.unsqueeze(1), dim=2)

    mask = 1 - torch.eye(num_embeddings, device=embeddings.device)
    pairwise_distances = pairwise_distances * mask

    distances = pairwise_distances.view(-1)
...
def UniformityLoss(proto_embedding):
    ...
    unit_matrix = torch.eye(cos_dist_matrix.shape[0]).to(proto_embedding.device)
...
def get_base_adj(adj, base_id, labels):
    ...
    return base_adj
def get_mask(I,labels,base_id,novel_idx):
    mask = []
    for i in range(I.shape[1]):
